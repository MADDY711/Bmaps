...
class Navigator:
    ...
    def _make_instruction(self, inst_type, message, obstacle=None):
        ...
        """
        # SPECIAL LOGIC: Silence 'CLEAR' unless it's a CHANGE in status
        if inst_type == "CLEAR":
            if self.last_instruction == "CLEAR":
                return None # Already clear, stay silent
            # If we're coming from an obstacle, we can say it once
            # but with a very long cooldown (10s)
            now = time.time()
            if now - self._last_speak_time < 10.0:
                return None

        # Apply cooldown for all other types (STOP, CAUTION, etc.)
